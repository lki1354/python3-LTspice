...
from LTspice.LTspiceAutomation import run_simulation
...
class ConverterSimulation(Circuit):
    ...
    def pars_log(self):
        self._log = open(os.path.normpath(
            self.simulation_dir + self.simulation_file.split('.')[0] + r'/' + self.simulation_file.split('.')[
                0] + r'.log'), encoding="utf-8").read()
        self.__read_values()
        print('pars finished!')
...
class Converter(ConverterData):
    ...
    def set_values(self, value_list):
        for name, value in value_list.items():
            self._circuit.set_value(name + '_v', value)
